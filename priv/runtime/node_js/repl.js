...
// node_modules/wetf/esm/packer.js
var Packer = class {
  constructor(options = {}) {
...
// node_modules/wetf/esm/unpacker.js
var Unpacker = class {
  constructor(options = {}) {
    ...
    switch (type) {
      ...
      case 111: {
        ...
        if (length < 7 || length === 7 && this._d[this._i + 6] < 32) {
          let num2 = 0;
          if (length === 1) {
            num2 = this._d[this._i];
          } else if (length === 2) {
            num2 = (this._d[this._i + 1] << 8) + this._d[this._i];
          } else if (length === 3) {
            num2 = (this._d[this._i + 2] << 16) + (this._d[this._i + 1] << 8) + this._d[this._i];
          } else if (length === 4) {
            num2 = this._v.getUint32(this._i, true);
          } else if (length === 5) {
            num2 = this._d[this._i + 4] * 4294967296 + this._v.getUint32(this._i, true);
          } else if (length === 6) {
            num2 = ((this._d[this._i + 5] << 8) + this._d[this._i + 4]) * 4294967296 + this._v.getUint32(this._i, true);
          } else {
            num2 = ((this._d[this._i + 6] << 16) + (this._d[this._i + 5] << 8) + this._d[this._i + 4]) * 4294967296 + this._v.getUint32(this._i, true);
          }
          this._i += length;
          if (sign) {
            num2 = -num2;
          }
          return this._safebigintDecoding === 3 ? num2.toString() : this._safebigintDecoding === 1 ? num2 : BigInt(num2);
        }
        let num;
        if (length === 8) {
...
// node_modules/wetf/esm/wetf.js
var wetf_default = { Packer, Unpacker };

// node_js/etf.js
var etfPacker = new wetf_default.Packer({
  encoding: {
...
var etfUnpacker = new wetf_default.Unpacker({
  decoding: {
...
async function execCall(encodedCall) {
  try {
    ...
    return etf_default.pack(["ok", result]);
  } catch ({ message, stack }) {
    return etf_default.pack(["error", { message, stack }]);
  }
}
...
var MODULE_SEARCH_PATHS = (process.env.NODE_PATH || "").split(import_node_path.default.delimiter).filter(Boolean);
async function importModuleRespectingNodePath(modulePath) {
  for (const searchPath of MODULE_SEARCH_PATHS) {
    const modulePathToTry = import_node_path.default.join(searchPath, modulePath);
...
async function fileExists(file) {
  return await import_promises.default.access(file, import_promises.default.constants.R_OK).then(() => true).catch(() => false);
}
